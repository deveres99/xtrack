...
class Extractioncheck(BeamElement):
    ...
    x_min : float
        Minimum x corrdinate to be considered extracted. Default is ``1``.
    x_max : float
        Maximum x corrdinate to be considered extracted. Default is ``1``.
    px_min : float
        Minimum px corrdinate to be considered extracted. Default is ``1``.
    px_max : float
        Maximum px corrdinate to be considered extracted. Default is ``1``.
    ...
    _extra_c_sources = [_pkg_root.joinpath('beam_elements/elements_src/extractioncheck.h')]

    def __init__(self, x_min = 1.,
                       x_max = 1.,
                       px_min = 1.,
                       px_max = 1.0, 
                       **kwargs):
        if '_xobject' not in kwargs:
            kwargs.setdefault('x_min', x_min)
            kwargs.setdefault('x_max', x_max)
            kwargs.setdefault('px_min', px_min)
            kwargs.setdefault('px_max', px_max)
